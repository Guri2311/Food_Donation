...
const sendMail = require("../utils/sendEmail"); // email utility
...
router.post("/donor/donate", middleware.ensureDonorLoggedIn, async (req, res) => {
	try {
		...
		// ✅ Send email to Admin notifying about the new donation
		const adminEmail = "gursimrankaur23004@gmail.com"; // 🔁 Replace with real admin email
		const subject = "New Donation Submitted";
		const message = `
A new donation has been submitted by ${req.user.email}.
Item: ${donation.item}
Quantity: ${donation.quantity}
Expiry Date: ${donation.expiry}
Please review it in the Admin Panel.
		`;

		await sendMail({ to: adminEmail, subject, text: message });
